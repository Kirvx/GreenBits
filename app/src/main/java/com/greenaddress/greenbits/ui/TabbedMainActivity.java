...
// Problem with the above is that in the horizontal orientation the tabs don't
// go in the top bar
public class TabbedMainActivity extends ActionBarActivity
    implements ActionBar.TabListener, Observer {
  ...
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    final boolean isBitcoinURL = Intent.ACTION_VIEW.equals(
        getIntent().getAction()) || NfcAdapter
    ...
    if (isBitcoinURL) {
      if (!getGAApp().getConnectionObservable()
          ...
          .equals(ConnectivityObservable.State.LOGGINGIN)) {

        // login required
        final Intent loginActivity = new Intent(this,
            RequestLoginActivity.class);
        startActivityForResult(loginActivity,
            REQUEST_BITCOIN_URL_LOGIN);
        return;
      }
    }
    launch(isBitcoinURL);
  ...
  private void launch(boolean isBitcoinURL) {
    ...
    if (isBitcoinURL) {
      if (NfcAdapter.ACTION_NDEF_DISCOVERED
        ...
        final Parcelable[] rawMessages = getIntent()
            .getParcelableArrayExtra(NfcAdapter.EXTRA_NDEF_MESSAGES);
        for (final Parcelable ndefMsg_ : rawMessages) {
          ...
          for (final NdefRecord record : ndefMsg.getRecords()) {
            if (record.getTnf() == NdefRecord.TNF_WELL_KNOWN
                ...
                NdefRecord.RTD_URI)) {
              mViewPager.setTag(R.id.tag_bitcoin_uri,
                  record.toUri());
            }
          }
        }
  ...
  public void onResume() {
    ...
    setIdVisible(getGAApp().getConnectionObservable().getState()
        != ConnectivityObservable.State.LOGGEDIN, R.id.action_share);
  }
  ...
  protected void onActivityResult(final int requestCode, final int resultCode,
    ...
    final String qrExtra = "com.greenaddress.greenbits.QrText";

    if (requestCode == REQUEST_SEND_QR_SCAN) {
      if (data != null && data
          .getStringExtra(qrExtra) != null) {
        String scanned = data.getStringExtra(qrExtra);
        if (!(scanned.length() >= 8 && scanned.substring(0, 8)
            .equalsIgnoreCase("bitcoin:"))) {
          scanned = String.format("bitcoin:%s", scanned);
        }
        final Intent browsable = new Intent(this,
      ...
      try {
        keyNonFinal = new DumpedPrivateKey(Network.NETWORK,
            data.getStringExtra(qrExtra)).getKey();
      } catch (final AddressFormatException e) {
        try {
          keyBip38NonFinal = new BIP38PrivateKey(Network.NETWORK,
              data.getStringExtra(qrExtra));
        } catch (final AddressFormatException e1) {
          Toast.makeText(TabbedMainActivity.this, getResources().getString(R
                  .string.invalid_key),
      ...
      final FutureCallback<Map<?, ?>> callback = new FutureCallback<Map<?,
        ...
        public void onSuccess(final @Nullable Map<?, ?> result) {
          final View inflatedLayout = getLayoutInflater().inflate(R.layout
              .dialog_sweep_address, null, false);
          final TextView passwordPrompt = (TextView) inflatedLayout
              .findViewById(R.id.sweepAddressPasswordPromptText);
          final TextView mainText = (TextView) inflatedLayout.findViewById(R
              .id.sweepAddressMainText);
          final TextView addressText = (TextView) inflatedLayout.findViewById
              (R.id.sweepAddressAddressText);
          final EditText passwordEdit = (EditText) inflatedLayout
              .findViewById(R.id.sweepAddressPasswordText);
          ...
          if (keyBip38 == null) {
            ...
            mainText.setText(Html.fromHtml("Are you sure you want to sweep " +
                "<b>all</b> ("
                + format.postfixCode().format(outputsValue) + ") funds from " +
                "the address below?"));
            address = keyNonBip38.toAddress(Network.NETWORK).toString();
          } else {
            passwordPrompt.setText(getResources().getString(R.string
                .sweep_bip38_passphrase_prompt));
            txNonBip38 = null;
            // amount not known until decrypted
            mainText.setText(Html.fromHtml("Are you sure you want to sweep " +
                "<b>all</b> funds from the password protected BIP38 key " +
                "below?"));
          ...
          new MaterialDialog.Builder(TabbedMainActivity.this)
              ...
              .callback(new MaterialDialog.ButtonCallback() {
                ...
                private void doSweep() {
                  final ArrayList<String> scripts = (ArrayList<String>)
                      result.get("prevout_scripts");
                  Futures.addCallback(getGAService().verifySpendableBy(
                    ...
                    public void onSuccess(@Nullable Boolean result) {
                      if (result.booleanValue()) {
                        final List<TransactionSignature> signatures = new
                            ArrayList<>();
                        for (int i = 0; i < tx.getInputs().size(); ++i) {
                          signatures.add(tx.calculateSignature(i, key, Hex
                              .decode(scripts.get(i)), Transaction.SigHash
                              .ALL, false));
                        }
                        Futures.addCallback(getGAService().sendTransaction
                            (signatures, null), new FutureCallback<String>() {
                          @Override
                          ...
                          public void onFailure(final Throwable t) {
                            t.printStackTrace();
                            Toast.makeText(TabbedMainActivity.this, t
                                .getMessage(), Toast.LENGTH_LONG).show();
                          }
                        });
                      } else {
                        Toast.makeText(TabbedMainActivity.this, "Verification" +
                            " failed: Invalid output address", Toast
                            .LENGTH_LONG).show();
                      }
                    }
                    ...
                    public void onFailure(final Throwable t) {
                      t.printStackTrace();
                      Toast.makeText(TabbedMainActivity.this, t.getMessage(),
                          Toast.LENGTH_LONG).show();
                    }
                  });
                }
                  ...
                  if (keyBip38 != null) {
                    try {
                      ...
                      Futures.addCallback(getGAService().prepareSweepSocial(
                          ...
                          ?>>() {
                        @Override
                        public void onSuccess(@Nullable final Map<?, ?>
                            result) {
                      ...
                      Toast.makeText(TabbedMainActivity.this, getResources()
                          .getString(R.string.invalid_passphrase), Toast
                          .LENGTH_LONG).show();
                    }
        ...
        @Override
        public void onFailure(final Throwable t) {
          Toast.makeText(TabbedMainActivity.this, t.getMessage(), Toast
              .LENGTH_LONG).show();
        }
      };
      if (keyBip38 == null) {
  ...
  public boolean onOptionsItemSelected(final MenuItem item) {
    ...
    if (id == R.id.action_settings) {
      final Intent settingsActivity = new Intent(TabbedMainActivity.this,
          SettingsActivity.class);
      startActivity(settingsActivity);
      return true;
    } else if (id == R.id.action_sweep) {
      final Intent scanner = new Intent(TabbedMainActivity.this, ScanActivity
          .class);
      startActivityForResult(scanner, REQUEST_SWEEP_PRIVKEY);
      return true;
    } else if (id == R.id.network_unavailable) {
      Toast.makeText(TabbedMainActivity.this, getGAApp()
          .getConnectionObservable().getState().toString(), Toast
          .LENGTH_LONG).show();
      return true;
      ...
      startActivity(new Intent(TabbedMainActivity.this, BitRefillActivity
          .class));
      ...
      final TextView receiveAddress = (TextView) findViewById(R.id
          .receiveAddressText);
      if (receiveAddress != null) {
        ...
        if (address != null && !address.isEmpty()) {
          ...
          sendIntent.putExtra(Intent.EXTRA_TEXT, BitcoinURI
              .convertToBitcoinURI(address.replace("\n", "").replace("\r",
                  ""), null, null, null));
  ...
  private void testKickedOut() {
    if (getGAApp().getConnectionObservable().getIsForcedLoggedOut() ||
        getGAApp().getConnectionObservable().getIsForcedTimeout()) {
      // FIXME: Should pass flag to activity so it shows it was forced logged
      // out
      final Intent firstScreenActivity = new Intent(TabbedMainActivity.this,
  ...
  public void update(final Observable observable, final Object data) {
    ...
    final ConnectivityObservable.State currentState = getGAApp()
        .getConnectionObservable().getState();
    setIdVisible(currentState != ConnectivityObservable.State.LOGGEDIN, R.id
        .network_unavailable);
  }
